<template>
  ...
  <div class="container-info" v-if="containerList.length > 0">
      ...
      <a-col :span="21" class="container-box">
        <div class="container-base-info">
          ...
          <template v-if="currentContainer.ACTION === 'leave'">
            {{
              getBoxByContainerRefId(currentContainer.CONTAINER_REF_ID)
                .containerInfo.PILE_PLACE
            }}
          </template>
        ...
        <div class="tag-wrp">
          <span class="import-tag">{{
            currentContainer.IMPORT_OR_EXPORT === "进" ? "Import" : "Export"
          }}</span>
          <span class="inside-tag">
            {{
              currentContainer.INSIDE_OR_OUTSIDE === "内"
                ? "Domestic"
                : "Foreign"
            }}
            Trade
          ...
          <span class="pack-tag">{{
            currentContainer.BILL_LADING_REF_ID
              ? container_bill_map[currentContainer["CONTAINER_REF_ID"]]
                  .length > 1
                ? "LCL"
                : "FCL"
              : "none"
          }}</span>
          <span class="used-tag" v-if="currentContainer.IF_USED === 'Y'"
            >Completed Docs</span
          >
          <span class="check-tag" v-if="currentContainer.IF_CHECK === 'Y'"
            >Customs Inspection</span
          >
          <span class="bind-tag" v-if="currentContainer.IF_BIND === 'Y'"
            >Binded Task</span
          >
          <span class="book-tag" v-if="currentContainer.IF_BOOK === 'Y'"
            >Booked InGate</span
          >
          <span class="action-tag">
            <template v-if="currentContainer.ACTION === 'move'">Move</template>
            <template v-if="currentContainer.ACTION === 'enter'"
              >Enter</template
            >
            <template v-if="currentContainer.ACTION === 'leave'"
              >Leave</template
    ...
    <div class="detail-info">
      ...
      <template
        v-if="
          (currentContainer.ACTION === 'move' ||
            currentContainer.ACTION === 'enter') &&
          currentContainer.IMPORT_OR_EXPORT === '进'
        "
      >
        <div class="detail-title">
          Predict Date：{{ currentContainer.PRE_DATE }}
        </div>
      </template>
      <div class="detail-title">
        Controlling Company：{{ currentContainer.CNTR_ADMIN_NAME }}
      </div>
      <div class="detail-title">
        Owning Company：{{ currentContainer.CNTR_OWNER_NAME }}
      </div>
      <template
        v-for="(item, index) in container_bill_map[
          currentContainer.CONTAINER_REF_ID
        ]"
        :key="index"
      >
        <div class="detail-title">
          Bill Lading Info：{{ item.BILL_LADING_NO }}
        </div>
        <div class="goods-box">
...
export default defineComponent({
      ...
      ws.onmessage = function (event) {
        if (event.data) {
          ...
          if (result.data.init) {
            ...
            const last_operation = JSON.parse(
              JSON.stringify(self.containerList[0])
            );
            ...
            if (result.data.action_info.success) {
              if (result.data.action_info.action_type === "move") {
                const currentBox = self.getBoxByContainerRefId(
                  last_operation["CONTAINER_REF_ID"]
                );
                currentBox.dispose();
                if (
                  result.data.action_info.enter_able_pile &&
                  self.currentContainer.ACTION === "move"
                ) {
                  result.data.action_info.enter_able_pile.forEach((pItem) => {
                    const temCtnInfo = JSON.parse(
                      JSON.stringify(self.currentContainer)
                    );
                    ...
                    material.alpha = 0.6;
                    material.diffuseColor = new BABYLON.Color4.FromHexString(
                      "#b8ff42FF"
                    );
                ...
                last_operation["PILE_PLACE"] =
                  result.data.action_info.pile_place;
                ...
                if (
                  result.data.action_info.enter_able_pile &&
                  self.currentContainer.ACTION === "enter"
                ) {
                  result.data.action_info.enter_able_pile.forEach((pItem) => {
                    const temCtnInfo = JSON.parse(
                      JSON.stringify(self.currentContainer)
              ...
              if (result.data.action_info.action_type === "leave") {
                ...
                result.data.action_info.block_container_id.forEach((cItem) => {
                  const currentBox = self.getBoxByContainerRefId(
                    cItem.CONTAINER_REF_ID
                  );
                  currentBox.material.diffuseColor = new BABYLON.Color3(
                    1,
                    0,
                    0
                  );
  ...
  methods: {
    curCtnColor() {
      const self = this;
      const currentBox = self.getBoxByContainerRefId(
        self.containerList[0].CONTAINER_REF_ID
      );
      if (self.containerList[0].ACTION === "leave") {
        currentBox.material.diffuseColor = new BABYLON.Color4.FromHexString(
          "#91003fff"
        );
    ...
    addContainer(container, material) {
      ...
      if (containerType === 20) {
        ...
        box.position.x =
          (-yard_width * box_width) / 2 +
          box_width / 2 +
          (bayCount - 1) * box_width;
        box.position.z = -(
          (-yard_height * box_depth) / 2 +
          box_depth / 2 +
          (containerRow - 1) * box_depth
        );
...
.container-info {
  ...
  right: 40px;
  bottom: 40px;
  background-color: rgba(75, 221, 255, 0.3);
  box-shadow: inset 0 0 30px rgba(75, 221, 255, 0.2);
